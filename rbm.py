...
class RBM(object):
    def __init__(self, n_input, n_hidden, layer_names, alpha=1.0, transfer_function=tf.nn.sigmoid):
        ...
        self.update_w = self.rbm_w + 0.95*alpha *((self.w_positive_grad - self.w_negative_grad) / tf.to_float(
            tf.shape(self.x)[0])+tf.matmul(0.01, self.rbm_w)) 
        self.update_vb = self.rbm_vb + 0.95*alpha * tf.reduce_mean(self.x - self.v1, 0)
        self.update_hb = self.rbm_hb + 0.95*alpha * tf.reduce_mean(self.h0prob  - self.h1, 0)
        ...
        self.sess.run(init)
    def impu_variables(self, batch):
      """
        1.compute the gradient of updation for missing variables
        2.(tf.reduce_sum(self.rbm_w[i])*self.rbm_w-tf.reduce_sum(self.update_w[i])*self.update_w
      """
        for(one batch)
        {
        for (n_input)
         {
         if (x_value == 'NA')
          delta_v= tf.matmul(self.rbm_w,self.rbm_w[i])-tf.matmul(self.update_w,self.update_w[i])
          self.update_vb= self.update_vb + tf.transpose(delta_v)
          }
        }
    ...
    def reconstruct(self, batch):
        return self.sess.run(self.v_sample, feed_dict={self.x: batch, self.rbm_w: self.o_w,
                                                      self.rbm_vb: self.o_vb, self.rbm_hb: self.o_hb})
    ...
    def _initialize_weights(self):
        ...
        all_weights = dict()
        all_weights['w'] = tf.Variable(tf.random_normal([self.n_input, self.n_hidden], stddev=0.01, dtype=tf.int32),
